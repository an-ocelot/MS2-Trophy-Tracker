...
function updater_() {
  ...
  for (i = 1; i < trophy_list.getRange('B1').getValue(); i++){
    ...
    //Check size of the sheet
    var size_c = parseInt(data.size_c);
    var size_r = parseInt(data.size_r);
    
    if (page.getMaxColumns() < size_c) page.insertColumns(page.getMaxColumns(), size_c - page.getMaxColumns());
    if (page.getMaxColumns() > size_c) page.deleteColumns(size_c + 1, page.getMaxColumns() - size_c);
    if (page.getMaxRows() < size_r) page.insertRows(page.getMaxRows(), size_r - page.getMaxRows());
    if (page.getMaxRows() > size_r) page.deleteRows(size_r + 1, page.getMaxRows() - size_r);
    ...
    for (category in data){          
      ...
      sidebar_bot.protect().setWarningOnly(true);
